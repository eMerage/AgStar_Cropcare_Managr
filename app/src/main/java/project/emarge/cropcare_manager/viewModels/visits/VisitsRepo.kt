package project.emarge.cropcare_manager.viewModels.visits
...
import project.emarge.cropcare_manager.model.datamodel.Image
import project.emarge.cropcare_manager.model.datamodel.Visits
import project.emarge.cropcare_manager.services.network.NetworkErrorHandler
import java.io.File
...
class VisitsRepo(application: Application) {
    ...
    fun visitsFilter(
        sDate: String,
        eDate: String,
        list: ArrayList<Visits>
    ): MutableLiveData<ArrayList<Visits>> {
        val dataVisits = MutableLiveData<ArrayList<Visits>>()
        var listVisits = ArrayList<Visits>()
        ...
        if (sDate != "" && eDate == "") {
            for (items in list) {
                if (items.visitsDate?.substring(0, 10).equals(sDate)) {
...
/*    private fun getImagesFromIntranalStorage(): Array<String?> {
        ...
        var cursor = app?.contentResolver?.query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, proj, null, null, orderBy)
        var count = cursor?.count
        var arrPath = arrayOfNulls<String>(count!!)
